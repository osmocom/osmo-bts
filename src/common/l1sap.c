				...
				struct ph_tch_param *rts_ind)
{
	...
	if (lchan->csd_mode == LCHAN_CSD_M_NT) {
		for (i = 0; i < 2; i++) {
			if (input_msg[i]) {
				ntcsd_dl_input_48(lchan, msgb_l2(input_msg[i]),
	...
	if (phy_msg) {
		...
		if (lchan->csd_mode == LCHAN_CSD_M_NT) {
			...
			for (i = 0; i < 2; i++) {
				...
				if (input_msg[i]) {
					memcpy(phy_data, msgb_l2(input_msg[i]),
						bits_per_20ms);
				} else {
				 ...
				 struct ph_tch_param *rts_ind)
{
	...
	if (csd_tchf48_nt_e2_map[fn % 26] == 0) {
		for (i = 0; i < 2; i++) {
			...
			input_msg = msgb_dequeue_count(&lchan->dl_tch_queue,
						       &lchan->dl_tch_queue_len);
			if (input_msg) {
				ntcsd_dl_input_48(lchan, msgb_l2(input_msg),
	...
	if (phy_msg) {
		resp_l1sap = msgb_l1sap_prim(phy_msg);
		phy_data = msgb_put(phy_msg, 120);
		if (csd_tchf48_nt_e2_map[fn % 26] == 0)
			memcpy(phy_data, rlp_frame, 120);
		else
			memcpy(phy_data, lchan->tch.csd.tchf48_nt_2ndhalf, 120);
	} else {
		resp_l1sap = &empty_l1sap;
	}
