...
rtp_abst_socket_create(void *talloc_ctx, bool use_twrtp,
			struct obts_twjit_config *twjit_cfg)
{
	...
	rs->twrtp = obts_twrtp_create(rs, 8, 20, true, twjit_cfg);
	if (rs->twrtp == NULL) {
		talloc_free(rs);
		return NULL;
	}
			 ...
			 int port)
{
	...
	rc = obts_twrtp_bind_local(rs->twrtp,
			(const struct osmo_sockaddr *) &rs->local_addr);
	if (rc < 0)
		return rc;
	return 0;
}
			    ...
			    const struct in_addr *ip, uint16_t port)
{
	...
	return obts_twrtp_set_remote(rs->twrtp,
...
#endif
	return obts_twrtp_set_dscp(rs->twrtp, dscp);
}
...
#endif
	return obts_twrtp_set_socket_prio(rs->twrtp, prio);
}
...
#endif
	return obts_twrtp_tx_quantum(rs->twrtp, payload, payload_len,
...
{
	const struct obts_twjit_rr_info *rri;
	int32_t cumulative_lost;

	rri = obts_twjit_get_rr_info(twjit);
...
#endif
	obts_twrtp_set_sdes(rs->twrtp, cname, name, email, phone,
			    loc, tool, note);
}
