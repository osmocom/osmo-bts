...
static void tx_vgcs_ul_free(struct gsm_lchan *lchan)
{
	/* FIXME: implement! */
}
...
static int tx_vgcs_ul_grant(struct gsm_lchan *lchan)
{
	...
	//return down_rsl(lchan->ts->trx, msg);
}
...
static void vgcs_t3115_cb(void *data)
{
	...
	if (lchan->state != LCHAN_S_ACTIVE) {
		LOGPLCHAN(lchan, DASCI, LOGL_NOTICE, "is in not active. It is in state %s. Ignoring\n",
			  gsm_lchans_name(lchan->state));
		return;
	}

	if (lchan->asci.vgcs_ul_grant_count >= bts->ny2) {
		tx_vgcs_ul_free(lchan);
...
{
	LOGPLCHAN(lchan, DASCI, LOGL_INFO, "First valid frame detected\n");
